...
public class DefaultFileHierarchySet implements FileHierarchySet {
    ...
    public static FileHierarchySet from(String absolutePath, MetadataSnapshot snapshot, CaseSensitivity caseSensitivity) {
        ...
        return new DefaultFileHierarchySet(snapshot.withPathToParent(relativePath.getAsString()), caseSensitivity);
    }
    ...
    public Optional<MetadataSnapshot> getMetadata(String absolutePath) {
        ...
        if (!relativePath.isPrefix(pathToParent, caseSensitivity)) {
            return Optional.empty();
        }
        return rootNode.getSnapshot(relativePath.withNewOffset(pathToParent.length() + PathUtil.descendantChildOffset(pathToParent)), caseSensitivity);
    }

    @Override
    public FileHierarchySet update(String absolutePath, MetadataSnapshot snapshot) {
        OffsetRelativePath relativePath = getRelativePath(absolutePath);
        return new DefaultFileHierarchySet(storeSingleChild(rootNode, relativePath, snapshot, caseSensitivity), caseSensitivity);
    }

    @Override
    public FileHierarchySet invalidate(String absolutePath) {
        String normalizedPath = normalizeRoot(absolutePath);
        OffsetRelativePath relativePath = getRelativePath(normalizedPath);
